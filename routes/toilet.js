...
router.get("/nearbyToilets", (req, res) => {
  ...
  if (!lng || !lat) {
    return res.status(422).json({ error: "Both lat and lng need to be present!" });
  }
...
router.get("/ownerToilets", requireLogin, async (req, res) => {
  try {
    const toiletArray = await Toilet.find({ owner: ObjectId(req.user._id) }).populate("owner");
    res.json(toiletArray);
  } catch (error) {
    console.log(error);
    res.status(422).json({ error: "Error fetching owner toilets" });
  }
});
...
router.get("/allToilets", async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(422).json({ error: "Error fetching all toilets" });
  }
});

router.post("/changeAvailability", requireLogin, (req, res) => {
  const { toilet_id } = req.body;
  ...
  Toilet.findOne({ _id: toilet_id }, function (err, toilet) {
    ...
    if (String(toilet.owner) !== String(req.user._id)) {
      return res.status(422).json({ error: "You do not own this toilet!" });
    }
...
router.post("/newRating", requireLogin, async (req, res) => {
  ...
  Toilet.findOne({ _id: toilet_id }, function (err, toilet) {
    if (!toilet) {
      return res.status(422).json({ error: "Toilet not found" });
    }
    ...
    toilet.avgRating = (toilet.avgRating * toilet.usersWhoRated.length + rating) / (toilet.usersWhoRated.length + 1);
    toilet.usersWhoRated.push(req.user._id);
...
router.post("/oneToilet", requireLogin, async (req, res) => {
  ...
  try {
    const theToiletDoc = await Toilet.findOne({ _id: toilet_id }).populate("owner");

    if (theToiletDoc) {
      res.json(theToiletDoc);
    } else {
      res.status(422).json({ error: "No such toilet exists" });
    }
  } catch (error) {
    console.log(error);
    res.status(422).json({ error: "Error fetching toilet" });
  }
});
